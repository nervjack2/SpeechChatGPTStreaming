...
class OnlineASRProcessor:
    ...
    def process_iter(self):
        ...
        #         print(w.word, w.start)

        # transform to [(beg,end,"word1"), ...]
        tsw = self.asr.ts_words(res)
        print("ASR (Audio buffer):", tsw)
        ...
        if len(self.audio_buffer)/self.SAMPLING_RATE > s:
            # print(f"trigger chunk: {len(self.audio_buffer)/self.SAMPLING_RATE:.2f}s")
        ...
        # print(f"len of buffer now: {len(self.audio_buffer)/self.SAMPLING_RATE:2.2f}")
        x = self.to_flush(o)
...
def asr_factory_from_config(config: dict, logfile=sys.stderr):
    """
    Creates and configures an ASR and ASR Online instance based on the specified backend and arguments.
    """
    ...
    if backend == "openai-api":
        logger.debug("Using OpenAI API.")
        asr = OpenaiApiASR(lan=config["language"])
    else:
        if backend == "faster-whisper":
            asr_cls = FasterWhisperASR
        else:
        ...
        size = config["model"]
        t = time.time()
        logger.info(f"Loading Whisper {size} model for {config['language']}...")
        asr = asr_cls(modelsize=size, lan=config["language"], cache_dir=None, model_dir=None)
        e = time.time()
        logger.info(f"done. It took {round(e-t,2)} seconds.")

    # Apply common configurations
    if config.get('vad', False):  # Checks if VAD argument is present and True
        logger.info("Setting VAD filter")
        asr.use_vad()
    ...
    #     tgt_language = "en"  # Whisper translates into English
    # else:
    tgt_language = config['language']  # Whisper transcribes in this language

    # Create the tokenizer
    if config["buffer_trimming"] == "sentence":
        tokenizer = create_tokenizer(tgt_language)
    else:
        tokenizer = None

    # Create the OnlineASRProcessor
    online = OnlineASRProcessor(asr,tokenizer,logfile=logfile,buffer_trimming=(config["buffer_trimming"], config["buffer_trimming_sec"]))
