...
from server.common.MindHandler import TextAudioMindHandler
...
class Processor(IProcessor):
    ...
    async def interruption(self, res):
        ...
        """
        await self.system_mind.suspend(new_timestamp=res["input_timestamp"])
    ...
    async def listen_for_llm_server(self):
        try:
            ...
            async for res in stream:
                if res["input_timestamp"] != self.system_mind.timestamp:
                    continue

                # v1.1
                if res["type"] == "system_text":
                    if not res.get("eos", False):
                        await self.system_mind.put_content({
                            "data": [(res["data"], False)],
                            "timestamps": None
                        })
                ...
                # >=v1.2
                    ...
                    if not res.get("eos", False):
                        ...
                        content = []
                        timestamps = []
                        for (x, is_tok) in zip(res["data"], res["token_mask"]):
                            if is_tok:
                                x = f"<|{x}|>"
                                t += 320 * Define.UNIT_DOWNSAMPLE_RATE
                            content.append((x, is_tok))
                            timestamps.append(t)
                        await self.system_mind.put_content({
                            "data": content,
    ...
    async def listen_for_tts_server(self):
        try:
            ...
            async for res in stream:
                if res["input_timestamp"] != self.system_mind.timestamp:
                    continue
                assert res["type"] == "system_audio"
                await self.system_mind.put_audio(res)
        except (ConnectionResetError, BrokenPipeError):
            print("Client connection closed")
            raise
        except Exception as e:
    ...
    async def _connect_mind_handler(self):
        """ connect to mind handler. """
        self.system_mind = TextAudioMindHandler()
        self.system_mind.audio_output_callback = self._audio_output_callback
        self.system_mind.content_output_callback = self._content_output_callback
    
    async def _content_output_callback(self, data: list, eos: bool):
        loop = asyncio.get_event_loop()
        input_timestamp = self.system_mind.timestamp
        if eos:
            await self.llm_client.send_data({
                "type": "assistant_said",
                "data": None,
                "eos": True
            })
            return
        if data:
            assistand_said = [x for (x, is_tok) in data]
            text = [x for (x, is_tok) in data if not is_tok]
            await self.llm_client.send_data({
                "type": "assistant_said",
                "data": "".join(assistand_said),
            })
            res = {
                "type": "system_text",
                "data": "".join(text),
    ...
    async def _audio_output_callback(self, data: np.ndarray, eos: bool):
        loop = asyncio.get_event_loop()
        input_timestamp = self.system_mind.timestamp
        res = {
            "type": "system_audio",
...
class MainServerProcessor(IServerProcessor):
    ...
    async def process(self, client_socket: socket.socket, addr):  # handle one client connection
        try:
            ...
            await processor._connect_mind_handler()
