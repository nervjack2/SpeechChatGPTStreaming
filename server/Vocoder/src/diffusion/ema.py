'''
ref: https://github.com/NVlabs/LSGM/blob/main/util/ema.py
'''
import torch
import warnings
...
class EMA(Optimizer):
    ...
    def swap_parameters_with_ema(self, store_params_in_ema):
        ...
        # stop here if we are not applying EMA
        if not self.apply_ema or not self.applied:
            warnings.warn('swap_parameters_with_ema was called when there is no EMA weights.')
            return

        for group in self.optimizer.param_groups:
            for i, p in enumerate(group['params']):
                if not p.requires_grad or not 'ema' in self.optimizer.state[p]:
                    continue
