...
class Processor(IProcessor):
    ...
    def _parse_generated_ids(self, generate_ids: list[int]):
        ...
        print("Decoded: ", token)
        kms, words = self.model._post_process(token)
        return remain_ids, kms, words, eos
    ...
    async def _infer_loop(self):
        ...
        while not eos_flag:
            ...
            remain_ids, kms, words, eos = self._parse_generated_ids(remain_ids)
            ...
            if words:
                ...
                await loop.sock_sendall(self.client_socket, length_prefixing(res_text))
            if kms:
                res_token = {
                    "type": "system_token",
                    "data": kms,
                ...
                await loop.sock_sendall(self.client_socket, length_prefixing(res_token))
